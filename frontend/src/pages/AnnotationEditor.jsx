...
import { ChevronLeft, ChevronRight, Plus, Trash2, Save, Download, Check, X, MousePointer2, ZoomIn, ZoomOut, Move, Square, ArrowLeft, FolderOpen, CheckCircle2, AlertCircle, Database } from 'lucide-react'
...
export default function AnnotationEditor() {
  ...
  const fetchIncludedCount = async () => {
    if (!currentProject) return;
    try {
        const { data } = await api.getImprovementStats(currentProject.project_id);
        setIncludedCount(data.included_frames || 0);
    } catch (e) {}
  }

  useEffect(() => { fetchIncludedCount() }, [currentProject])
    ...
    try {
      ...
      if (!data.is_annotated) {
          ...
          initialBoxes = initialBoxes.filter(b => {
              const thresh = savedThresholds[b.class_name] ?? 0.50; 
              return b.confidence >= thresh;
          });
      }
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex flex-col gap-4 relative">
      ...
      {notification && (
          <div className={`fixed top-4 right-4 z-[99999] px-6 py-4 rounded-lg shadow-2xl border flex items-center gap-3 animate-in slide-in-from-right duration-300 ${
      ...
      <div className="flex items-center justify-between bg-slate-800/80 p-4 rounded-xl border border-slate-700">
          ...
          <div className="flex items-center gap-3">
              ...
              <div className="bg-emerald-900/40 border border-emerald-500/50 text-emerald-400 px-3 py-2 rounded-lg flex items-center gap-2 font-bold text-sm shadow-sm mr-2">
                  <Database size={16} />
                  {includedCount} In Dataset
              </div>
                ...
                {boxes.map((box, i) => {
                  ...
                  return (
                    <div
                        ...
                        selectedBoxId === i ? 'bg-slate-700/80 shadow-md' : 'bg-slate-900 border-slate-700 hover:border-slate-500'
                        }`}
                        style={{ borderColor: selectedBoxId === i ? boxColor : '' }}
                    >
                        <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <div className="w-3 h-3 rounded-full" style={{ backgroundColor: boxColor }}></div>
                            <div>
...
function AnnotationCanvas({ imagePath, boxes, setBoxes, selectedBoxId, onSelectBox, drawMode, newBoxClassId, classNameMap }) {
  ...
  const [newBoxPx, setNewBoxPx] = useState(null) 
            ...
            <Layer>
                ...
                {isDrawing && newBoxPx && (
                    <Rect
                        ...
                        stroke={CLASS_COLORS[newBoxClassId % CLASS_COLORS.length]} 
                        strokeWidth={2 / scale}
                        dash={[5, 5]}
                    />
                )}
...
const BoundingBoxShape = ({ shapeProps, isSelected, boxColor, onSelect, onDragEnd, onTransformEnd }) => {
  ...
  return (
    <>
      <Rect
        ...
        strokeWidth={isSelected ? 3 / (shapeRef.current?.getStage()?.scaleX() || 1) : 2 / (shapeRef.current?.getStage()?.scaleX() || 1)} 
        fill={isSelected ? hexToRgba(boxColor, 0.4) : hexToRgba(boxColor, 0.15)}
      />
      {isSelected && (
        <Transformer
          ref={trRef}
